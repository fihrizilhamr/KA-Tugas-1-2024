...
resource "null_resource" "install_lamp" {
  provisioner "local-exec" {
    command = <<EOT
      sudo apt-get install -y apache2 mysql-server php libapache2-mod-php php-mysql phpmyadmin git unzip
    EOT
  }
}
...
resource "null_resource" "remove_web_root" {
  provisioner "local-exec" {
    command = "sudo rm -rf ${var.web_root}"
  }
}
...
resource "null_resource" "clone_repo" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      sudo chmod -R 0755 ${var.web_root}
    EOT
  }
}
...
resource "null_resource" "set_document_root" {
  provisioner "local-exec" {
    command = <<EOT
      sudo sed -i 's|DocumentRoot .*|DocumentRoot ${var.web_root}|' /etc/apache2/sites-available/000-default.conf
    EOT
  }
}
